...
{	
	...
	strukturaAdresu->sin_addr.s_addr=inet_addr(adresHosta);
 	if (strukturaAdresu->sin_addr.s_addr==INADDR_NONE)
 	{
  	...
  	if(!(serwer=getservbyname(usluga, "UDP")))
  	{
...
int uzyskajGniazdoUDP()
{
	/*struct protoent *protokol;
	protokol=getprotobyname("UDP");*/
  	return socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);//protokol->p_proto);
}
...
int main(int argc, char *argv[])
{
	...
	if(argc!=5)
	{
		printf("Błąd użycia programu.\nPoprawne użycie: UDP_klient [adres IPv4] [port] [czas oczekiwania(s)] [porcja danych(B)]\n");
		return 0;
	}
	
	uzyskaneGniazdo=uzyskajGniazdoUDP();
	...
	while(1)
	{
		...
		wyslaneBajty=sendto(uzyskaneGniazdo, buforPorcjiDanych, porcjaDanych, 0,
						(const struct sockaddr *) strukturaAdresu,(socklen_t) sizeof(*strukturaAdresu));
		if(wyslaneBajty==-1)
		{
			printf("Błąd wysyłania do %s:%d (%s).\n", inet_ntoa(strukturaAdresu->sin_addr), ntohs(strukturaAdresu->sin_port), strerror(errno));
			break;
		}
		fprintf(stderr,"Wysłałem porcję danych o rozmiarze: %d do %s:%d\n", wyslaneBajty, inet_ntoa(strukturaAdresu->sin_addr), ntohs(strukturaAdresu->sin_port));
