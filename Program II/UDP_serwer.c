...
struct sockaddr_in *uzyskajStruktureAdresuSerwera(struct sockaddr_in *strukturaAdresu, char *port)
{
   ...
   strukturaAdresu->sin_addr.s_addr=htonl(INADDR_ANY);
	return strukturaAdresu;
}
...
int main(int argc, char *argv[])
{
	...
	if(argc!=4)
		printf("Błąd użycia programu.\nPoprawne użycie: UDP_serwer [port] [czas oczekiwania(s)] [porcja danych(B)]\n");
	...
	while(1)
	{
		...
		else */
		{
			...
			do
			{
				...
				odebraneBajty=recvfrom(uzyskaneGniazdo, buforPorcjiDanych, porcjaOdbierana,0,
									(struct sockaddr *) strukturaAdresuPol, &dlugoscAdresuPolaczonego);
				...
				fprintf(stderr, "Odebrałem %d bajtów danych od %s:%s.\n",odebraneBajty,"0.0.0.0","0");
